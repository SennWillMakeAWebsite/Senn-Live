...
function loadState() {
  try {
    ...
    console.warn(
      "Senn Live: failed to load state.",
      error
    );
...
function saveState() {
  try {
    ...
    console.warn(
      "Senn Live: failed to save state.",
      error
    );
  }
}
...
function detectPreviousSession() {
  ...
  try {
    ...
    if (
      ...
      previousLastActive = oldState.lastActive;
    }

  } catch (error) {
    console.warn(
      "Senn Live: previous session check failed."
    );
  }
}
...
function setStatus(status, logActivity = false) {
  ...
  /*
   * Only automatic status changes can
   * create Recent Activity entries.
   *
   * Manual buttons use logActivity = false.
   */
...
function renderStatus() {
  ...
  elements.statusIcon.style.background =
    getStatusBackground(state.status);

  elements.statusIcon.style.borderColor =
    getStatusBorder(state.status);

  const dot =
    elements.statusIcon.querySelector("span");
...
function updateHeartbeat() {
  ...
  if (difference < 15000) {
    ...
    elements.heartbeat.textContent =
      "Healthy";
...
function addActivity(status, label) {
  const now = Date.now();
  ...
  /*
   * Keep only the latest 12 activities.
   */
  state.activity =
...
function registerActivity() {
  ...
  /*
   * If the user was Idle or Offline,
   * automatically return to Online
   * and log only that real transition.
   */
...
elements.statusButtons.forEach(button => {
  ...
  button.addEventListener(
    ...
    () => {
      ...
      /*
       * Do not create activity log here.
       */
      resetIdleTimer();
...
}, 10000);  activityList: document.getElementById("activityList"),
  clearActivity: document.getElementById("clearActivity"),
...
}, 10000);  heartbeatDetail: document.getElementById("heartbeatDetail"),
